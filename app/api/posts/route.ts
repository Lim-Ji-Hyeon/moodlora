...
export async function GET(request: NextRequest) {
  ...
  // ─── 신고 5건+ 게시글 자동 제외 (로그인 여부 무관) ──────────────────────────
  const { data: allReports } = await supabase.from("reports").select("post_id");
  if (allReports && allReports.length > 0) {
    const reportCount: Record<string, number> = {};
    for (const r of allReports) {
      if (!r.post_id) continue;
      reportCount[r.post_id] = (reportCount[r.post_id] ?? 0) + 1;
    }
    const autoHiddenIds = Object.entries(reportCount)
      .filter(([, c]) => c >= 5)
      .map(([id]) => id);
    if (autoHiddenIds.length > 0) {
      hiddenPostIds = [...new Set([...hiddenPostIds, ...autoHiddenIds])];
    }
  }
  ...
  if (tags) {
    ...
    if (tagNames.length > 0) {
      ...
      filteredPostIds = ptData
        ? [...new Set(ptData.map((pt) => pt.post_id))]
        : [];
    }
  }
