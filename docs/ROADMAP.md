...
- [x] `/profile` 페이지 구현 (본인 프로필: 닉네임, 내 게시글 목록 최신순)
- [x] 닉네임 수정 기능 (Zod 검증: 2~20자, 특수문자 제외)
- [x] `/profile/[userId]` 동적 라우트 구현 (`params` await 필수, 타인 프로필: 공개 게시글만 표시)
- [x] 익명으로 작성한 게시글은 프로필에서도 "익명" 처리
- [x] Playwright MCP 테스트: 프로필 조회, 닉네임 수정 API 확인
- [x] **반응형 (내 프로필)**: Mobile(단일 컬럼) / Desktop(좌측 프로필 카드 + 우측 게시글 목록 2컬럼)
- [x] **반응형 (타인 프로필)**: Mobile 전용 (큰 화면에서 중앙 정렬 단일 컬럼)
...
- [x] 스켈레톤 UI 전체 페이지 적용 (피드, 상세, 댓글, 히스토리, 프로필)
- [x] Error Boundary 전역 및 페이지별 설정 (`error.tsx`, `not-found.tsx`)
- [x] 빈 상태 UI 통일 (게시글 없음, 댓글 없음, 검색 결과 없음, 히스토리 없음)
- [x] 로딩/에러/빈 상태 토스트 메시지 통일
- [x] 키보드 내비게이션 전체 검수 (감정 선택, 공감 버튼, 폼 제출 등)
