...
- [ ] `/welcome` 페이지 서버 컴포넌트 구현 (이미 감정 선택된 경우 `/feed` 리다이렉트)
- [ ] `EmotionPicker` 클라이언트 컴포넌트 구현 (7가지 감정 카드: 감정명, 이모지, 색상 표시)
- [ ] 감정 카드 선택 시 Zustand 스토어 업데이트 + 쿠키 저장 + `/feed` 라우팅
- [ ] 키보드 내비게이션 지원 (감정 카드 Tab/Enter 접근)
- [ ] 색각이상자 고려: 이모지 병기로 색상 외 구분 수단 제공
- [ ] 모바일/데스크탑 반응형 그리드 레이아웃
...
- [ ] `/login` 페이지 구현 (Google / Kakao 로그인 버튼 노출)
- [ ] 로그인 성공 후 이전 페이지 또는 `/feed`로 리다이렉트
- [ ] 비로그인 탐색 안내 메시지 ("로그인 없이도 피드를 둘러볼 수 있어요")
...
- [ ] 감정 필터 바 구현 (7가지 감정 칩, 선택 시 URL `searchParams` 업데이트 및 피드 재조회)
...
- [ ] Step 1: 감정 선택 (현재 Zustand 감정이 기본값, `EmotionPicker` 재활용)
- [ ] Step 2: 상황 태그 선택 (1개 이상 ~ 5개 이하, 선택 수 표시)
- [ ] Step 3: 본문 입력 (10~1,000자, 실시간 글자 수 카운터) + 익명 토글
- [ ] react-hook-form + Zod resolver로 단계별 유효성 검증
- [ ] 제출 성공 시 작성된 게시글 상세 페이지(`/posts/[id]`)로 이동
- [ ] 제출 실패 시 에러 토스트 표시, 폼 데이터 유지
- [ ] TanStack Query `useMutation`으로 서버 요청 및 피드 캐시 무효화
